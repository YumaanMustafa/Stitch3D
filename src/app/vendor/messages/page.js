...
import { MessageSquare, Send, Search, RefreshCw, UserCheck } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

/**
 * @file page.js
 * @description Vendor Inbox - Simplified Text & Modern Light Theme.
 */
        ...
        <div className="flex flex-col h-[calc(100vh-160px)] animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                <div>
                   ...
                   <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic">Customer Chat</h1>
                   <p className="text-sm font-medium text-slate-500 mt-2">Chat with your customers about their orders and designs.</p>
                </div>
            </div>
                    ...
                    <div className="flex-1 overflow-y-auto scrollbar-hide">
                        {loading ? (
                            ...
                            filteredContacts.map(contact => (
                                ...
                                >
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black shadow-lg transition-transform group-hover:scale-105 ${selectedContact?.id === contact.id ? 'bg-slate-900' : 'bg-slate-300'}`}>
                                    ...
                                    <div className="text-left flex-1 min-w-0">
                                        ...
                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Customer</p>
                                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        {selectedContact ? (
                                ...
                                <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-2xl bg-slate-900 flex items-center justify-center text-[#F97316] font-black shadow-lg">
                                            {selectedContact.first_name?.[0]?.toUpperCase() || selectedContact.email[0].toUpperCase()}
                                        </div>
                                        <div>
                                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-tighter italic">
                                                {selectedContact.first_name ? `${selectedContact.first_name} ${selectedContact.last_name}` : selectedContact.email.split('@')[0]}
                                            </h3>
                            ...
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col items-center justify-center text-slate-400">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest">Select a customer to start chatting</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
